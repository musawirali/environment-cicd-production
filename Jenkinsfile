      ...
      steps {
        container('maven') {
          dir('env') {
            ...
            sh '. ./awscredentials.env; set -x && aws rds create-db-snapshot --region us-west-2 --db-instance-identifier jerry-stage-jx --db-snapshot-identifier $(echo "promotion@$(date \'+%m%d%Y-%H:%M:%S\')")'
